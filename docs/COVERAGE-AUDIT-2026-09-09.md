...
| 3 — in spec, not implemented, milestone already assigned | ~30 control groups | M6/M7/M9/M10/M11/M12/M13 work not yet started or UI-only |
...
concept explicitly; folding it into M13 (polish) is the natural home, not
a fresh milestone.

## 1.2 Exact signal flow

Mixed states, item by item (hardware strip order): source (state 1) →
pre-fader tap (state 3, M10/M7) → insert point (state 3, M9/2.3's AUv3
...
output delay (state 3, M11/M4-adjacent, no dedicated owner yet, folding
into M4's clocking work or M13 is reasonable) → device output (state 1).
...
home is M5 (already merged) retroactively, or M13 as a small addendum —
...
milestone owner (M13 polish is the natural home given its
...
knobs: state 3 (M9). Monitor select: state 3 (M8/M13-adjacent UI work, no
...
State 3, M10, not started. `SPEC.md`'s "one or all inputs" phrasing
...
State 3, spread across several milestones (M11 for MIDI/network dialogs,
M13 for the rest) — not implemented, correctly scoped, no changes needed.

## 1.11 System settings dialog, every field

Mostly state 3 (M4/M11/M13, not yet built as a dialog). Two additions
needed:
...
State 3, M13, not started. Matches extraction exactly (64 slots, F1-F24,
...
State 3 across the board (M13 for most, M4's own virtual driver control
...
**M11** (the milestone that already owns macro buttons' system actions).
...
- **A vendor naming reuse, not a bug**: the 4 VBAN-TEXT output stream
  ...
  Noted for anyone implementing M11/M12 against these names later, not a
  `SPEC.md` change.
