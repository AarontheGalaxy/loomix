...
**UI status (M10):** on/off, A/B, the six cells, per-channel `Trim`, per-channel `Delay`, `FLAT` and `CH COPY` are all reachable from the panel as of M10 — all seven were already implemented and tested in the engine before M10, so leaving any of them unwired would have reopened the exact state-2 gap M10 exists to close (`docs/COVERAGE-AUDIT-2026-09-09.md`). Three items from this list remain genuinely unreachable and are deliberately deferred, each to a real milestone rather than silently dropped: `COPY ALL` (a different command shape from `CH COPY` — it copies between two separate `ParametricEq` instances, strip-to-bus or bus-to-bus, not one channel to another inside the same instance), loading and saving the whole EQ set as a file (real file I/O and a save/open dialog, not just wiring an existing pure function), and the two right-click gestures (typing an exact value, changing the graph's dB scale — spec 1.18 interaction conventions that cut across many controls, not specific to this panel). **Milestone: M15**, the same one that already owns the rest of spec 1.18's interaction polish and (for the file case) sits closest to preset/file work.
...
**M11 — Device robustness: genuine sample-rate mismatches and degraded devices.** Found live against real hardware, not in review: connecting a Bluetooth headset (AirPods) for capture drove the underrun counter at roughly 20,000/second, unbounded, because `DriftCorrectedIoStage` (spec 2.3, M4) has no way to represent anything but a small correction around a ratio of exactly 1.0 — right for clock drift between two devices already at the same nominal rate, structurally unable to reach a genuine rate difference (AirPods' HFP capture runs at 24kHz nominal; a typical master output runs 44.1kHz or higher, an 84% mismatch, worse than the 2026-08-28 log's own worked 44.1/48kHz example that already proved unreachable).

**Design decision, made explicit before implementation:** Loomix resamples properly across a genuine nominal-rate mismatch; it does not refuse the pairing. The resampler (`resample.rs`, M4) already accepts an arbitrary ratio per call and needs no change — a large-ratio *upsample* (the direction a low-rate capture device needs to reach a higher-rate master) is exactly what an interpolating sinc kernel handles well, since the source is already band-limited below its own Nyquist by construction; nothing here is downsampling, which is the direction that would actually need a matched anti-alias filter this resampler doesn't have. The fix is entirely in how the *ratio fed to it* gets computed: `DriftCorrectedIoStage` gains a `base_ratio` field, set once at connect time from a plain division (`master_sample_rate / device_nominal_sample_rate`, both already queried — `connect_audio` must now also read the *input* device's own nominal rate, which today it never does, the other half of the 2026-08-28 gap) — no control loop, because the ratio a genuine rate difference needs is a known, fixed quantity the moment both devices report their own rates, not something to be discovered by feedback. The existing PI controller and its discontinuity detection (`drift.rs`) are unchanged: their output, always `1.0 ± max_correction`, becomes a small multiplicative correction *around* `base_ratio` instead of *being* the final ratio, so they keep doing exactly the small-residual-drift job they were built and already proven for, now measured against the true baseline instead of a wrong one. This is the "fixed ratio outside the drift loop" the M4 log's own gap entry left as the open question, answered: `base_ratio` lives in `ioproc.rs`, computed once outside `drift.rs`; `drift.rs` itself needs no changes at all. Because `DriftCorrectedIoStage` already serves both `on_capture` and `on_render`, this fix covers a future non-master render device's own rate mismatch automatically, without a second design pass when that capability is wired.

A bound still applies: a computed ratio that's non-finite, zero, or outside `[1/32, 32]` is refused with a clear error rather than resampled — a Loomix-own sanity margin (not vendor-sourced; no coverage found in `docs/audit/` for this scenario at all, plausibly because Windows exposes a Bluetooth headset's HFP/A2DP modes as distinct, explicitly-chosen endpoints rather than one endpoint whose effective rate changes live), generous enough to cover any real device pairing (8kHz telephony-grade hardware against a 192kHz interface is a 24x span) while still catching a genuinely invalid property read.

**Also in this milestone: the degraded-device warning.** When a connected or selected device's real channel count is lower than the bus feeding it, or its nominal rate reveals a reduced-bandwidth profile (the Bluetooth HFP case above), the UI says so plainly — at device-selection time and on the connected-status line — instead of presenting a silently degraded connection as a normal one. **Acceptance:** the exact real-hardware scenario this milestone is named for (a genuine Bluetooth-class capture-rate mismatch against a real master) settles to near-zero underruns after connecting rather than climbing without bound, proven against the same synthetic-hardware test harness `ioproc.rs`'s existing suite already uses, not just reasoned about; every existing `drift.rs` test stays green unmodified, proving the residual-drift logic genuinely wasn't touched; and a device offering fewer channels than its bus, or a reduced Bluetooth profile, is visibly flagged in the running app, not just logged.

**M12 — Recorder.** Playback and recording, all source modes, formats, multitrack, pre-record buffer, timer.

**M13 — Control surface.** Request script parser, unix socket, C ABI, CLI, MIDI mapping with learn and feedback, macro buttons with all trigger types including the audio level trigger.

**M14 — Network audio.** Incoming and outgoing streams, discovery, jitter buffer, error indicators, text and MIDI sub protocols. Fuzz the parser before merging.

**M15 — Polish and release.** Preset scenes, simplified remote view, menu bar mode, login item, first run wizard, signed and notarised installer, uninstaller, documentation site.

**M16 — Final manual verification (release gate).** A full manual re-audit against all three vendor manuals in `~/Documents/loomix-refs/`, repeating the 2026-09-09 audit's methodology exactly: each manual read in full, in non-overlapping, page-cited chunks, extraction kept as a separate pass from classification, checked against `docs/SPEC.md` and the running app itself rather than the code in isolation. Every feature in Part 1 must be confirmed present, reachable from the UI, and actually working end to end — not merely implemented. **This is the release gate: no `v*` tag and no `release.yml` run until this milestone's audit reports zero open findings.**
