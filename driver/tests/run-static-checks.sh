...
# analyzer plus a clang-tidy pass, over every .c file in the driver
# target. Both are required checks -- a missing tool fails the run rather
...
for source in "$driver_dir"/*.c; do
    echo "== clang static analyzer: $(basename "$source") =="
    clang \
        --analyze \
    ...
    echo "== clang-tidy: $(basename "$source") =="
    "$clang_tidy" "$source" -- -std=gnu17 -isysroot "$sdk"
done
